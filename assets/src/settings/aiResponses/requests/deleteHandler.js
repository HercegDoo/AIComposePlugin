...
export function handleDelete(id) {
  ...
  deleteButton.onclick = () => {
    ...
    rcmail.show_popup_dialog(
      translation("ai_predefined_popup_body"),
      translation("ai_predefined_popup_title"),
      buttons,
      {
        height: 30,
      }
    ...
    const deleteButtonClickListener = () => {
      rcmail
        ...
        .done(function (data) {
          if (data.status === "success") {
            rcmail.display_message(
              translation("ai_predefined_successful_delete"),
              "confirmation"
            );
            getPredefinedInstructions();
          } else {
            rcmail.display_message(
              `${translation("ai_predefined_unsuccessful_delete")} ${data.message}`,
              "error"
            );
          }
        })
        .fail(function () {
          rcmail.display_message(
            translation("ai_predefined_unsuccessful_delete"),
            "error"
          );
        });
    };
