...
- **Full email translation:** In an opened message, use **Translate** beside Roundcube's Summary, Headers, and format links. A compact menu lets you choose a language and start translation. The complete text then appears above the unchanged original, with progress for long messages and controls to hide or retry the translation. Users can show or hide this control in Settings independently of summaries.
...
With `aiSummaryEnabled = true`, opened-message summaries adapt to the message length: messages under 100 words get one sentence, messages from 100 to 149 words get up to two, and messages of 150 words or more get up to three. By default, the summary appears only when the extracted message body contains at least 150 words, including quoted conversation when present. Roundcube checks the opened message length before sending the page. In long-message mode, qualifying messages show the summary card immediately with a **Generating summary…** indicator; messages under 150 words never insert the card or contact the AI provider automatically. Instead, **Show summary** appears beside **Translate** in the message header and can generate a summary on demand without changing the saved display mode. If the page cannot determine eligibility, the card stays hidden until the summary request confirms the message is long enough. Enable the hover preview in Settings to show a short translated summary when hovering over a message row. The opened-message summary includes earlier conversation context only when needed to explain the latest request or decision. **Show original** reveals the summary in the detected source language, and **Translate again** requests a fresh result. By default, the plugin uses the active Roundcube session language; changing the UI language yields a new cached translation. Summaries are generated on demand and cached for seven days in Roundcube's per-user database cache. Refresh bypasses that cache.
...
Opened messages show a compact **Translate** control in the message header links beside Summary, Headers, and Plain text. Open it to choose any language installed in Roundcube and click **Translate full email**. The initial choice follows the summary language setting when it names a specific language; otherwise it follows the active Roundcube interface language. The toolbar control takes no space above the message body until translation starts. Progress appears briefly above the original message, and the completed translation appears there with a **Hide translation** button. Reopen the menu to show a hidden translation or translate again. Changing the target language clears the previous result. Translation starts only when clicked, independently of summary visibility and `aiSummaryEnabled`.

In **Settings → AICompose Settings**, set **Full email translation in opened messages** to **Show** or **Hide**. It is shown by default, including for existing users. Hiding it removes the toolbar control and blocks translation requests for that user; it does not change either summary view. The administrator can disable it for everyone with `aiTranslationEnabled = false`.
