...
- **Incoming email summaries and translation:** Hover over a message for a short preview or open it for a longer summary that adapts to message length. Each view can be turned off independently in Settings. The plugin detects the source language and can show the summary in the active Roundcube language, another installed language, or the original language. Users can reveal the original summary or refresh it; results are cached per user for seven days.
...
- **Roundcube integration:** The interface uses the Elastic skin, localized labels, and light and dark theme styling. Compose options, summary visibility, and summary language preferences belong to the signed-in user.
...
In **Settings → AICompose Settings**, set **AI summary on message hover** and **AI summary in opened messages** independently to Show or Hide. Both are shown by default for existing and new users. Hiding one view stops its summary requests while the other view remains available; hiding the opened-message summary also removes its reply suggestions. You can choose the active Roundcube interface language (default), any language installed in that Roundcube instance, or the message's original language without translation. The language choice applies to both enabled views, and the cache separates these choices. The former style, length, creativity, and compose-language controls have moved out of Settings; change them in the Compose AI options to save them automatically to your Roundcube account. The saved choices are used on the next compose page load, including on another browser or device.
