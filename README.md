...
With `aiSummaryEnabled = true`, opened-message summaries adapt to the message length: messages under 100 words get one sentence, messages from 100 to 199 words get up to two, and messages of 200 words or more get up to three. By default, the summary appears only when the extracted message body contains at least 200 words, including quoted conversation when present. The summary card appears immediately with a **Generating summary…** indicator while one server request checks the message length and prepares the summary. For messages under 200 words in long-message mode, the card is removed and no AI provider request is made. Enable the hover preview in Settings to show a short translated summary when hovering over a message row. The opened-message summary includes earlier conversation context only when needed to explain the latest request or decision. **Show original** reveals the summary in the detected source language, and **Translate again** requests a fresh result. By default, the plugin uses the active Roundcube session language; changing the UI language yields a new cached translation. Summaries are generated on demand and cached for seven days in Roundcube's per-user database cache. Refresh bypasses that cache.
