...
- **Incoming email summaries and translation:** Enable hover previews in Settings for a short summary, or open a message for a longer summary that adapts to message length. Each view can be controlled independently. The plugin detects the source language and can show the summary in the active Roundcube language, another installed language, or the original language. Users can reveal the original summary or refresh it; results are cached per user for seven days.
