...
- Summaries are cached in Roundcube's per-user database cache for seven days, keyed by message identity, locale, provider configuration, prompt version, and sentence count. A short opened message may reuse the hover summary; longer messages have separate cache entries. Refresh bypasses the cache. Do not store raw incoming content in cache or log it.
