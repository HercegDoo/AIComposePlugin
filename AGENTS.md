...
4. `Settings` reads user defaults from the Roundcube `aicDefaults` preference (`style`, `length`, `creativity`, `language`, `pluginVisibility`, `summaryLanguage`). The Settings page edits visibility and summary language; `SaveComposeOptionsAction` reads the `data[aic][...]` form shape used by the v2 Settings page, saves compose choices, and preserves all other `aicDefaults` keys. It also accepts the earlier flat AJAX fields. New compose pages render the server preferences directly and `GetComposeOptionsAction` refreshes those choices from the signed-in user's server preferences after the page loads. Saved instructions use a separate `predefinedInstructions` preference; each record has `id`, `title`, and `message`. Use Roundcube's `get_prefs()` and `save_prefs()` and keep data scoped to the signed-in user.
