...
- `MailTask` loads `assets/dist/summary.bundle.js` with a content hash URL on the Elastic mail list when hover summaries are enabled and on opened messages when either the opened-message summary or full-message translation is enabled. It exposes `aiSummaryViews` and `aiSummaryMessageMode` from the signed-in user's `summaryHover` and `summaryMessage` preferences. Hover defaults to `hide`; opened-message summary defaults to `long` when no choice was saved. Existing saved `show` (always) and `hide` choices remain in effect. `summaryMessage` accepts `hide`, `long`, or `show`; the other visibility controls accept only `hide` or `show`. In `long` mode, the frontend first sends `check: 1` to `SummarizeMessageAction` to read the signed-in message and determine eligibility without contacting a provider. At 200 or more extracted body words (including quoted conversation when present), it inserts the card with a fetching indicator before sending the normal summary request. Shorter bodies leave no card; the normal action also returns `status: skipped` before contacting a provider if the body is too short, guarding against changes between requests. The card is followed by a separate reply suggestions panel when suggestions exist. Requests identify `preview` or `message`; the server validates that value. For opened messages, `SummaryPromptBuilder::sentenceCountForBody()` selects one, two, or three sentences based on the extracted body word count (under 100, 100–199, or 200+ words). Hover previews always use one sentence. Opened messages may also show one to three reply suggestions, or none when the intent is unclear.
