...
- Summaries are cached in Roundcube's per-user database cache for seven days, keyed by message identity, locale, provider configuration, prompt version, view, and sentence count. Hover and opened-message summaries have separate cache entries because only opened messages request reply suggestions. Refresh bypasses the cache. Do not store raw incoming content in cache or log it.
- Clicking a reply suggestion calls `plugin.aicomposeplugin_PrepareSuggestedReplyAction`, which places the selected instruction in `ReplySuggestionStore` under a one-time token for five minutes. Roundcube carries `_aic_reply_token` into its compose session. `MailTask::attachSuggestedReply()` checks the token against the reply UID and mailbox before exposing the instruction to `assets/src/compose.js`; that script calls the existing `generatemail` command. Never put the suggestion text in a compose URL or log it. The draft must remain editable and must never be sent automatically.
