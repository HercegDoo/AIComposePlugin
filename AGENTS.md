...
- `aiDebugLogging` is disabled by default. When enabled in the plugin config, `Debug/RequestLogger` writes complete prompts and provider token usage to Roundcube's `aicomposeplugin_ai` log with a request ID, purpose, timing, and safe response metadata. OpenAI and Ollama use the same logger. Cached summaries cause no provider request and therefore no token record.
...
- Never log API keys or full provider responses. Prompts, previous conversations, email content, and Sent examples may appear only in the dedicated AI request log when `aiDebugLogging` is explicitly enabled. Keep debug logging disabled by default; do not copy prompt data into generic error logs or caches. The OpenAI transport verifies TLS certificates and uses a fixed 60-second timeout; preserve certificate verification when changing providers.
