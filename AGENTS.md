...
4. `Settings` reads user defaults from the Roundcube `aicDefaults` preference (`style`, `length`, `creativity`, `language`, `pluginVisibility`, `summaryLanguage`, `summaryHover`, `summaryMessage`, `translationMessage`). The Settings page edits visibility, summary language, the two independent summary views, and full-message translation visibility; `SaveComposeOptionsAction` reads the `data[aic][...]` form shape used by the v2 Settings page, saves compose choices, and preserves all other `aicDefaults` keys. It also accepts the earlier flat AJAX fields. New compose pages render the server preferences directly. The frontend does not call `GetComposeOptionsAction` or replace the rendered choices after page load. Saved instructions use a separate `predefinedInstructions` preference; each record has `id`, `title`, and `message`. Use Roundcube's `get_prefs()` and `save_prefs()` and keep data scoped to the signed-in user.
...
1. `MailTask` registers `assets/dist/composeOptions.bundle.js` and `assets/dist/compose.bundle.js` in the compose startup hook and again when rendering the compose template as a fallback; Roundcube deduplicates the same URLs. Content hash queries make browsers fetch updated builds even when copied files retain their modification time. It sets `rcmail.env.aiPluginOptions` and `aiPredefinedInstructions`, and injects templates through `ContentInjector`.
2. `assets/src/composeOptions.js` initializes the explicit **Save as default** button in the AI compose options. `assets/src/compose/emailHelpers/composeOptionsPersistence.mjs` installs one capturing document click listener before Roundcube renders the sidebar. Clicking the button reads all four current selects and sends one same-origin POST in the v2 `data[aic][...]` shape, with Roundcube's request token header. Select `input` and `change` events do not save. The button stays disabled while the request is in flight, and Roundcube shows a confirmation only after a successful server response; failures show a warning. New compose pages render saved Roundcube preferences directly. The compose options do not use browser storage or inline JavaScript. `assets/src/compose.js` initializes the other compose commands. `assets/src/compose/commands/sendPostRequest.js` collects data through `emailHelpers/`, sends an `rcmail.http_post` request, and inserts the response into the plain-text or TinyMCE editor. The request sends `htmlMode` from `rcmail.editor.is_html()`; `RequestData` carries it to the shared prompt builder. HTML responses are sanitized to a small TinyMCE-compatible tag set in `emailHelpers/htmlEmail.js` before insertion. Generated replies are cleaned of any model-copied conversation before removing a model-generated closing when Roundcube already has a signature; do not remove Roundcube's existing quote or signature from the editor. `signatureUtils.mjs` handles plain text and `htmlEmail.js` handles HTML fragments. Requested length counts visible words, while the API token cap still includes markup.
...
- `MailTask` loads `assets/dist/summary.bundle.js` with a content hash URL on the Elastic mail list when hover summaries are enabled and on opened messages when either the opened-message summary or full-message translation is enabled. It exposes `aiSummaryViews` and `aiSummaryMessageMode` from the signed-in user's `summaryHover` and `summaryMessage` preferences. Hover defaults to `hide`; opened-message summary defaults to `long` when no choice was saved. Existing saved `show` (always) and `hide` choices remain in effect. `summaryMessage` accepts `hide`, `long`, or `show`; the other visibility controls accept only `hide` or `show`. On opened messages, the Roundcube `message_load` hook uses the already loaded `rcube_message` to set `aiSummaryAutoEligible` before HTML is sent. In `long` mode, the frontend inserts the card with a generating indicator only when eligibility is true; short messages never insert the card and instead show a manual header action beside Translate. When eligibility is unknown, it keeps the card detached until the summary action confirms the message is long enough. The action still returns `status: skipped` for extracted bodies under 150 words (including quoted conversation when present), before contacting a provider. The card remains visible while a qualifying summary is generated. The manual action requests a short message summary with `force: 1`; the server accepts that override only for the `message` view and still rejects it when opened-message summaries are hidden. The force flag does not change the saved preference. The card is followed by a separate reply suggestions panel when suggestions exist. Requests identify `preview` or `message`; the server validates that value. For opened messages, `SummaryPromptBuilder::sentenceCountForBody()` selects one, two, or three sentences based on the extracted body word count (under 100, 100–149, or 150+ words). Hover previews always use one sentence. Opened messages may also show one to three reply suggestions, or none when the intent is unclear.
- `plugin.aicomposeplugin_SummarizeMessageAction` accepts a message UID, mailbox, optional refresh flag, and optional `view` (`preview` by default or `message`). It rejects a view disabled in the user's preferences before reading mail or contacting a provider. The target language comes from the signed-in user's validated `summaryLanguage` preference: current Roundcube interface language by default, another installed Roundcube locale, or original language without translation. It never comes from the client request. It reads the message through Roundcube's signed-in mail storage, strips HTML, and sends up to 12,000 characters plus subject to the selected provider.
...
- `aiTranslationEnabled` is the administrator switch for full-message translation. The signed-in user's `aicDefaults.translationMessage` preference independently shows or hides the compact Translate control in the opened message's `#message-header .header-links`; it defaults to `show` for existing and new users. `assets/src/summary/translation.js` puts the language menu in a fixed popover and inserts progress or the completed result into `#message-content .rightcol` only after the user starts translation. `MailTask` gates the bundle and environment on both switches, and `TranslateMessageAction` enforces both switches before reading mail or contacting a provider. The action validates message UID, mailbox, installed Roundcube target locale, and part index, then includes the subject with the body. `MessageTranslationText` converts the entire plain or HTML message body to text and splits text into 3,500-character parts without dropping characters. Messages over 60,000 text characters or 1 MB of raw body data are rejected, with no silent truncation. `TranslationService` asks the same provider configured for summaries to translate one part per request. The browser assembles results only after all parts succeed and renders them as text above the unchanged original. Full translations are not stored in the summary database cache; images and attachments are outside the translation scope. Ollama's output budget can be overridden for this action.
- `config.inc.php.dist` exposes `aiSummaryEnabled`, `aiTranslationEnabled`, `aiSummaryProvider`, `aiSummaryOpenAIConfig`, `aiSummaryGeminiConfig`, and `aiSummaryOllamaConfig`. OpenAI and Gemini summary and translation requests inherit their respective compose configuration unless overridden. Ollama is only used for incoming-message summaries and translation.
...
- Webpack entry points are `assets/src/compose.js`, `assets/src/composeOptions.js`, `assets/src/settings.js`, and `assets/src/summary.js`. JavaScript uses the Roundcube `rcmail`/`rcube_webmail` globals, and compose code also uses TinyMCE. Check both plain-text and HTML editor behavior when changing insertion or text revision.
- HTML templates use Roundcube `<roundcube:...>` tags. `ContentInjector` looks for specific IDs in rendered `elastic` HTML (`composebodycontainer`, `compose-options`, `headers-menu`, `layout-content`). `.select-div` is the parent of the AI select controls in the rendered HTML; `#compose-options` is its preceding Roundcube sibling. JavaScript expects IDs including `aic-instruction`, `aic-generate-email-button`, `aic_style_select`, `aic_length_select`, `aic_creativity_select`, `aic_language_select`, `aic-save-compose-options`, `composebody`, and `responses-table`. Carry ID or selector changes through PHP, HTML, JavaScript, and a manual Roundcube check.
