...
1. `MailTask` registers `assets/dist/compose.bundle.js` in the compose startup hook and again when rendering the compose template as a fallback; Roundcube deduplicates the same URL. A content hash query makes browsers fetch updated builds even when copied files retain their modification time. It sets `rcmail.env.aiPluginOptions` and `aiPredefinedInstructions`, and injects templates through `ContentInjector`.
2. `assets/src/compose.js` initializes commands, loads saved compose choices with `rcmail.http_get`, and binds a delegated `change` listener on the document so style, length, creativity, and language selects still save if Roundcube inserts them after initialization. Each change sends only its changed field through `rcmail.http_post` in the v2 `data[aic][...]` shape; changes made while the initial load is pending save immediately, and the load does not replace a changed control. In-flight saves are serialized and pending field changes are coalesced. The compose options do not use browser storage. `assets/src/compose/commands/sendPostRequest.js` collects data through `emailHelpers/`, sends an `rcmail.http_post` request, and inserts the response into the plain-text or TinyMCE editor. The request sends `htmlMode` from `rcmail.editor.is_html()`; `RequestData` carries it to the shared prompt builder. HTML responses are sanitized to a small TinyMCE-compatible tag set in `emailHelpers/htmlEmail.js` before insertion. Generated replies are cleaned of any model-copied conversation before removing a model-generated closing when Roundcube already has a signature; do not remove Roundcube's existing quote or signature from the editor. `signatureUtils.mjs` handles plain text and `htmlEmail.js` handles HTML fragments. Requested length counts visible words, while the API token cap still includes markup.
